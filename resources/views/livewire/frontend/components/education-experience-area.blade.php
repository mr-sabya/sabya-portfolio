...
<section class="education-experience tmp-section-gapTop">
    <div class="container">
        ...
        {{-- SECTION 1: NOW SHOWING EXPERIENCES --}}
        ...
        <div class="row g-5">
            ...
            <div class="col-lg-6 col-sm-6">
                <div class="education-experience-card tmponhover tmp-scroll-trigger tmp-fade-in animation-order-{{ $loop->iteration }}">
                    <h4 class="edu-sub-title">{{ $exp->designation }}</h4>
                    <h2 class="edu-title">{{ $exp->name }} ({{ $exp->duration }})</h2>
                    <p class="edu-para">{{ $exp->description }}</p>
                </div>
            </div>
            @endforeach
        </div>

        {{-- SECTION 2: NOW SHOWING EDUCATION --}}
